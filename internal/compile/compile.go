...
package compile // import "github.com/spachava753/starlarkx/internal/compile"
...
func (fcomp *fcomp) expr(e syntax.Expr) {
	...
	case *syntax.DictExpr:
		...
		for _, entry := range e.List {
			fcomp.emit(DUP)
			if unpack, ok := entry.(*syntax.UnaryExpr); ok {
				fcomp.expr(unpack.X)
				fcomp.setPos(unpack.OpPos)
				fcomp.emit(DICTMERGE)
				continue
			}
