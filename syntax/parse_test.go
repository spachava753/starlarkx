...
func TestExprParseTrees(t *testing.T) {
	for _, test := range []struct {
		...
		{`x.load()`,
			`(CallExpr Fn=(DotExpr X=x Name=load))`},
		{`x.class`, `not an identifier`},
		{`x.if`, `not an identifier`},
		{`load`, `got load, want primary expression`},
		{`x+y*z`,
			`(BinaryExpr X=x Op=+ Y=(BinaryExpr X=y Op=* Y=z))`},
		{`x%y-z`,
			`(BinaryExpr X=(BinaryExpr X=x Op=% Y=y) Op=- Y=z)`},
		{`a + b not in c`,
			`(CompareExpr X=(BinaryExpr X=a Op=+ Y=b) List=((Comparison Op=not in Y=c)))`},
		{`a < b <= c != d`,
			`(CompareExpr X=a List=((Comparison Op=< Y=b) (Comparison Op=<= Y=c) (Comparison Op=!= Y=d)))`},
		...
		{`{x for x in xs if x for y in ys}`,
			`(Comprehension Curly Body=x Clauses=((ForClause Vars=x X=xs) (IfClause Cond=x) (ForClause Vars=y X=ys)))`},
		{`{x}`, `(SetExpr List=(x))`},
		{`{x, y}`, `(SetExpr List=(x y))`},
		{`{x for x in xs,}`, `got ',', want '}', for, or if`},
		{`{x: y for (x, y) in z}`,
