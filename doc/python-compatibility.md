...
| Values / Range arithmetic limits | `STARLARKX` | `DEFAULT` | `YES` | Keep constructor arguments and range lengths within signed machine integers. Reject construction when the exact length exceeds that limit. Compute slices without wrapping derived parameters or materializing elements, retaining arbitrary-precision slice metadata when necessary. Keep existing slice-argument type and width restrictions unchanged. | Preserve compact ordinary ranges while preventing silent data loss and panics at integer boundaries. Slice metadata may exceed constructor limits because even a one-element slice can have an oversized step. |
