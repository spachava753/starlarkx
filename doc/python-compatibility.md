...
| `map` strict option | Does not accept `strict`; stops at the shortest input. | Accepts keyword-only `strict=False`; with `True`, unequal input lengths cause an error during iteration. | Restriction |
| `zip` strict option | Does not accept `strict`; stops at the shortest input. | Accepts keyword-only `strict=False`; with `True`, unequal input lengths cause an error during iteration. | Restriction |
